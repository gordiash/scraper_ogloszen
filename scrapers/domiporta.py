...
def get_domiporta_listings(max_pages: int = 3) -> List[Dict]:
    """
    Pobiera ogłoszenia z Domiporta.pl
    ...
    for page in range(1, max_pages + 1):
        try:
            ...
            logger.info(f"Scrapuję Domiporta.pl - strona {page}")
            
            soup = get_soup(url)
            
            # Selektory dla Domiporta
            offers = (soup.select(".listing-item") or 
...
def parse_domiporta_listing(offer_element) -> Dict:
    ...
    """
    # Tytuł
    title_elem = (offer_element.select_one(".listing-title") or
    ...
    # Cena
    price_elem = (offer_element.select_one(".listing-price") or
    ...
    # Lokalizacja
    location_elem = (offer_element.select_one(".listing-location") or
