...
def get_gratka_listings(max_pages: int = 3) -> List[Dict]:
    """
    Pobiera ogłoszenia z Gratka.pl
    ...
    """
    listings = []
    base_url = "https://gratka.pl/nieruchomosci/mieszkania/sprzedam"
    
    for page in range(1, max_pages + 1):
        try:
            url = f"{base_url}?page={page}"
            logger.info(f"Scrapuję Gratka.pl - strona {page}")
            
            soup = get_soup(url)
            
            # Selektory dla Gratka
            offers = (soup.select(".teaserUnified") or 
...
def parse_gratka_listing(offer_element) -> Dict:
    ...
    """
    # Tytuł
    title_elem = (offer_element.select_one(".teaserUnified__title") or
    ...
    # Cena
    price_elem = (offer_element.select_one(".teaserUnified__price") or
    ...
    # Lokalizacja
    location_elem = (offer_element.select_one(".teaserUnified__location") or
    ...
    # Powierzchnia
    area_elem = (offer_element.select_one(".teaserUnified__area") or
                 offer_element.select_one(".area"))
    area_text = clean_text(area_elem.get_text()) if area_elem else ""
    
    # Liczba pokoi
    rooms_elem = (offer_element.select_one(".teaserUnified__rooms") or
                  offer_element.select_one(".rooms"))
    rooms_text = clean_text(rooms_elem.get_text()) if rooms_elem else ""
    
    # Opis (skrócony)
    desc_elem = offer_element.select_one(".teaserUnified__description")
